...
> **実装状況(2026-07-24追記)**: 4節の後続分解のうち、Follow-up A(責務9)はIssue #176、
> Follow-up B(責務10: `confirmedClusterIsMutable`/`evaluatePostConfirmationLifecycle`)と
> Follow-up C(standingPartyの縮小・解散・再探索ライフサイクル本体)はIssue #177で実装済み
> (`formationPolicy.ts`/`engine.ts`、テストは`clusterDeparture.test.ts`・
> `standingPartyClusterLifecycle.test.ts`)。3.1節の型シグネチャ案とは、`evaluatePostConfirmationLifecycle`
> が候補ではなく`GroupCandidate`側の`everConfirmed`フラグ(3.4節で検討候補として挙げられていたもの)を
> 実際に採用した点が異なる――責務3の成立判定(近接ヒューリスティック)はconfirmed遷移直後の
> `memberIds`が成立最小人数に届いていないことがあるため、実際に一度でも届いた後の減少だけを
