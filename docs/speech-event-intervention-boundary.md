...
- **SpeechEventの生成経路は`engine.ts`に一本化されている。** `speech.ts`の`createSpeechEvent`(唯一のコンストラクタ)を呼ぶ箇所は`engine.ts`内に限られ、次の2パターンのみ:
  ...
  なお、Phase 4(Issue #115)の乖離調整(`applyPublicExpressionsToSpeech`、socialExpression.ts)は
  この基礎生成の**後段**で適用される追加ステップであり、生成経路の2パターン・二重生成禁止の規則は変えない
  (idも基礎生成時のまま変更しない)。
- **`reason: "lightObserverInvitation"`のSpeechEventは、Phase 4(Issue #115)の乖離調整の対象外である。**
  この発言は介入シナリオの意図(observerJoinerへ確実に声を届け、後押し効果を発生させる)がトリガーであり、
  話者個人の対外表現スタンス(遠慮・同調圧力による抑制/軟化)より介入の意図を優先する。
  Phase 4有効時もintent(`invite`)は置換・抑制されず、`SpeechEvent.expression`(乖離スナップショット参照)も
  付与されない。既存の効果経路(`Agent.invitedAtTick`/`isUnderLightInvitationBoost`)にも一切影響しない。
