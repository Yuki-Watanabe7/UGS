...
- **会場退出**(`leave`タグ、`state: "leaving"`→`"left"`): 二次会シナリオと同じ、会場そのものから
  離れる動きです。立食パーティーのPhase 1挙動では、この2つを状態ログ・タグ・Canvas表示のいずれでも
  混同しないようにしています。

### Phase 1の離脱判断について(暫定ルール)

輪からの離脱(責務9)は、`docs/interaction-cluster-model.md`に記載のとおり、一定tick以上の滞在後に
確率的に発生する暫定ルールです。現実の性格特性(会話への満足度・社交性・ObserverJoinerとしての葛藤など)
をモデル化したものではなく、**性格診断の結果ではありません**。誰かが輪から離脱しやすい/しにくいことを、
その人の社交性や性格の優劣として解釈しないでください。

### Phase 2以降の予定

Phase 2以降では、この暫定離脱ルールを、会話への満足度・社交性・ObserverJoiner固有の葛藤といった、
より意味のあるモデルへ置き換え・拡張する予定です(接触ネットワーク図・会話相手履歴の本格的タイムライン・
cluster間移動の統計ダッシュボード・現実データによる較正は、Phase 1・本節の対象外です)。
