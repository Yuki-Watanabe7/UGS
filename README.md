...
## 教室での班分けプリセット(classroom-pair系列)

上記5つの二次会プリセットとは別に、「先生が教室で班を作るよう指示する」という別の場を観察するための
4つのプリセットです。いずれも`formationScenarioId: "classroomPair"`の
[`FormationPolicy`](src/simulation/formationPolicy.ts)へ切り替わり、二次会シナリオとは異なる成立・終了
ルールでシミュレーションが進みますが、Start/Pause/Step・Monte Carlo・observerJoinerインスペクター・
ログフィルタは二次会プリセットと共通で使えます。

- **6. 教室で自由にペアを作る場**(`classroom-pair`): 2人固定(`minGroupSize = maxGroupSize = 2`)
- **7. 教室で自由に3人班を作る場**(`classroom-group-3`): 3人固定
- **8. 教室で自由に4人班を作る場**(`classroom-group-4`): 4人固定
- **9. 教室で自由に3〜4人班を作る場**(`classroom-group-3-4`): 3人で成立するが4人まで受け入れる可変定員

**目的**: 定員が固定された(二次会のように無制限に混ざれない)状況で、同時に複数の班形成が競合し、
参加に失敗した人が再探索する、という力学を観察します。observerJoiner相当の人(自分から誘わず、
誘われるのを待ちやすい人)がこの制約下でどうなりやすいかも、二次会シナリオでの孤立と対比して観察できます。
3人固定・4人固定は人数によって構造的な未割当が生じ得る一方、3〜4人可変は班サイズを組み合わせられるため
未割当が生じにくい、という定員ルールの違いによる帰結の対比も観察できます。

**起動方法**: 画面左側のプリセット選択で上記いずれかを選び、通常どおりStart/Step/Monte Carloを実行する
だけです。人数を増やしたい場合はpopulationSizeスライダーを変更してReset(Resetで反映)。

### 二次会シナリオとの違い

| | 二次会(afterParty、プリセット1〜5) | 教室での班分け(classroomPair、プリセット6〜9) |
| --- | --- | --- |
| 定員 | 実質無制限(`groupConfirmSize`人以上集まれば成立し、以後も自由に混ざれる) | プリセットごとの固定/可変定員(`minGroupSize`/`maxGroupSize`)。上限に達すると満員(full)で新規参加不可 |
...
| 未割当の扱い | 未割当という終端状態は存在しない(最終的に`joined`か`left`のいずれか) | 締切時点で班が決まっていない人は`unassigned`という終端状態になる。定員によっては構造的に必然的な未割当が生じる |
...
### 班人数バリエーションと空き・満員・未割当の意味

- **min(成立最小人数)**: この人数以上集まると班が`confirmed`(成立)になる。固定定員では`min === max`。
- **max(収容最大人数)**: 班がこれ以上は受け入れられない上限。可変定員(3〜4人班)では成立後も
  `max`に達するまで新規参加を受け付け続ける(「空きのある成立済み班」)。
- **満員(full)**: `confirmed`かつ`memberIds.length === max`になった状態。新規参加はできない
  (`groupPartition.ts`/`formationPolicy.ts`のテストで、満員候補への参加不可を保証している)。
- **構造的未割当**: 締切までに班に入れなかった人。固定定員では人口が定員で割り切れない場合に
  必然的に生じる(例: 3人固定で人口20人なら最大2人)。可変定員(3〜4人)は複数のサイズを
  組み合わせられるため、同じ人口でも構造的未割当が生じないことがある(`groupPartition.ts`の
  `computeStructuralUnassignedCount`が、この人数を定員ルールから決定的に計算する)。
  未割当は個人の失敗ではなく、定員という数学的制約と締切という時間的制約の帰結であり、
  「探索に失敗した」ことと「構造的にそもそも入りきらなかった」ことは区別して見る必要がある。

### 教師介入(学校シナリオ専用)

二次会シナリオの介入(集合場所の明示、声かけなど)とは別に、教室での班分けシナリオには教師視点の
介入が6種類あり、介入セレクターから選択できます(いずれも`classroomPair`シナリオ専用で、二次会
シナリオで選んでもno-op)。

| 介入 | 分類 | 何をするか |
| --- | --- | --- |
| 近くの人への声かけ促進 | 低圧 | 組み合わせは指定せず、近接する未決定同士に一時的な後押しを与えるだけ。直接は組ませない |
| 空きのある班の参加可能表示 | 公開情報 | 空きのある班を「まだ入れます」と可視化し、未決定者の気づきを助ける |
| 匿名の支援要請通知 | 匿名通知 | 長時間困っている生徒を、個人を特定しない形で教師へ知らせる(通知だけでは結果は変わらない) |
| 教師による候補推薦 | 推薦 | 教師が班や相手を推薦するが、受け入れるかどうかは本人のwillingness等に委ねる(直接所属はさせない) |
| 締切時の教師強制割当 | 強制割当 | 締切時点の未割当者を、定員内で教師が可能な限り割り当てる。既存班の再配分が発生し得る |
| seed付きランダム割当(比較基準) | 比較基準 | 教師の救済ではなく「自由形成そのものを行わない」比較用の基準点。tick 0で決定的にシャッフルして定員通りに分割する |

推薦・強制割当・ランダム割当のいずれも、対象選定や受諾判定は本体のシミュレーション用乱数
(`SeededRandom`)とは独立した介入専用の乱数(`createInterventionRandom`)を使うため、
介入の有無や種類を変えても本体の乱数消費系列(=agentの動き自体)は変化しません。各介入の発火・
対象・受諾/拒否・強制割当/再配分・割当不能はすべて構造化イベント(`state.log`の
`schoolInterventionTriggered`等)として記録され、監査できます。実装詳細は
[学校向け介入実行契約の設計](docs/school-intervention-hooks.md)を参照してください。

### 学校シナリオのpaired Monte Carlo比較

二次会の介入あり/なし比較(`InterventionComparisonPanel`)と同様に、教室での班分けシナリオにも
介入なし(baseline)と介入ありをseed列を揃えて比較する`GroupFormationComparisonPanel`があります
(`compareGroupFormation`、Issue #160)。比較時は次の点に注意する。

- **未割当人数の減少だけを見ない**: 強制割当は未割当を減らせるが本人の選択を上書きする。stress・
  参加失敗回数(`searchRestartCount`/`capacityFailureCount`)・強制割当された人数もあわせて見る。
- **ランダム割当は自由形成と単純比較できない**: 接近・参加失敗・再探索という探索過程自体を経ない
  ため、他の介入と同じ過程指標(平均成立tickなど)で並べても意味が異なる比較になる。あくまで
  「探索過程を省いた場合の割当結果」を見るための基準点として扱う。
...
(班未割当)のまま残ることは、その人の人格や社交性の欠陥を意味しません。モデル化された特性
(`initiative`/`willingness`/`influenceAvoidance`など)・既存の人間関係(`cliqueId`/`existingTieStrength`)・
定員という構造的制約・時間制約(`formationDeadlineTick`)・(有効化していれば)教師介入という複数要因が
組み合わさった結果として観察される、モデル上の一つの帰結です。「待ちやすい」「未割当になりやすい」は
固定の属性値ではなく、関係性・時間・定員・他者の行動・介入の有無という相互作用の帰結として解釈してください。
固定定員で人口が定員で割り切れない場合は、どんなに特性を調整しても構造的な未割当自体は避けられません
(`docs/core-agent-dynamics.md`の[6.3節](docs/core-agent-dynamics.md#63-学校のペア形成ポリシー)を参照)。

教師強制割当・ランダム割当についても、どちらが「優れているか」をこのシミュレーションが自動的に判定する
ことはありません。強制割当は未割当を減らす一方で本人の選択を上書きし、ランダム割当は探索負担そのものが
存在しないため他の介入と同じ過程指標で単純比較できない、という性質の違いがあるだけです。

### 既知の制約

現時点の教室での班分けプリセットには、次の制約があります(対象外/将来の拡張候補であり、実装済みではありません)。

- **複数介入の組み合わせ探索は対象外**: 上記6種類の教師介入は同時に1つだけ選択する前提で、
  複数介入を組み合わせた場合の相互作用は扱っていません。
- **教師介入の倫理審査・推奨順位の自動化は対象外**: どの介入を使うべきかの倫理的判断・優劣付けは
  行いません。構造化イベントによる監査可能性を提供するのみです。
- **現実データによる較正は未実装**: 二次会シナリオと同様、係数やしきい値は「挙動の対比が観察できる」
  ことを基準に調整した値であり、実際の教室でのペア形成・班分けに関する実証データによる較正は行って
  いません。較正前の数値を現実の介入効果として扱わないでください。
- **大規模性能最適化は対象外**: 明確な性能回帰が見つかった場合は別Issue化します。

## 立食パーティー(standingParty)プリセット — Phase 1

会場のあちこちで複数の会話の輪が並行して形成される立食パーティーのシナリオです。二次会・教室とは異なり、
輪が成立した後もシミュレーションは終わらず、人はその輪から離脱して再探索したり、別の輪へ再参加したり、
輪自体も人数が減れば縮小・解散したりを繰り返します(#173〜#177で実装、本節は#178でのPhase 1統合確認)。
...
1tickごとに、次の循環が起こり得ます。

1. **cluster作成**: 主導者(`initiative >= 0.5`)、または近くに集まった既存の仲良しグループが、新しい
   「会話の輪」(`forming`状態の`GroupCandidate`)を作り始めます。
2. **会話成立**: 輪の周辺に十分な人数が集まると`confirmed`(会話中)になります。
3. **参加**: 未定の人が輪に近づき、合流します(`forming`/`confirmed`いずれの輪にも合流できます)。
4. **離脱**: `confirmed`な輪に合流した人は、一定tick以上その場に留まった後、確率的にその輪を離れ、
   `undecided`(再探索中)へ戻ります。
5. **再探索・再参加**: 離脱した人は、直前に離れた輪へすぐには戻らない短いクールダウンを挟みつつ、
   同じ輪または別の輪を再び探し、合流し直します。
6. **縮小・解散**: `confirmed`な輪は離脱によって人数が減ります。成立最小人数を下回ると解散し、
   残っていた人は孤立させず全員再探索状態へ戻します(0人になった場合は猶予なく即解散)。

この一連の流れは、構造化イベント(`nucleusCreated`/`groupConfirmed`/`agentJoined`/
`clusterDepartureStarted`/`clusterDepartureCompleted`/`clusterResearchStarted`/`clusterRejoined`/
`activeClusterShrunk`/`activeClusterDissolving`/`activeClusterDissolved`/`clusterMemberReleased`)から、
表示文言の文字列解析なしに追跡できます。状態ログの「輪の離脱・再探索・人数変動のみ」フィルタで、
これらのイベントだけに絞り込んで確認できます。Canvasでは、成立済みの輪の枠が破線(mutable)になり、
「完了して固定されたグループ」ではなく人数が変動しうる輪であることを示します。直前に輪を離脱して
再探索クールダウン中の人は、黄色い破線の縁取りで区別されます。ObserverJoinerインスペクターでは、
現在の輪の状態・人数・滞在tick・直前に離脱した輪・離脱→再探索の累計回数を確認できます。

### 観測horizonと自然終了の違い

二次会・教室のシナリオは、全員の所属が確定する・全クラスタが成立する・締切tickに達するなど、
意味論的な条件で自然に終了します。立食パーティーはそのような自然終了を持ちません(輪の縮小・解散・
再探索が繰り返されること自体がこのシナリオの意図した挙動のため)。そのため対話的なUIでは、
ユーザーが手動でpause/resume/resetするまで実行が続きます。バッチ実行・自動テスト・Monte Carlo集計では、
代わりに`observationHorizonTick`(観測期間の上限tick)を指定でき、到達すると`simulationFinished`
(`finishReason: "observationHorizonReached"`)として打ち切ります。これは「輪が成立した」「全員落ち着いた」
という意味の終了ではなく、あくまで「ここまで観察した」という区切りである点に注意してください。

### cluster離脱と会場退出の違い

立食パーティーには、意味の異なる2種類の「輪から離れる」があります。

- **クラスタ離脱**(`clusterDeparture`タグ): 会話の輪そのものから離れて再探索する動きです。会場には
  残っており、`state`は`undecided`(再探索中)に戻ります。ログでは「輪を離れ始めた」「輪から離れた」
  「新しい輪を探し始めた」のように表現され、「帰宅」「退出」「参加をやめた」とは表現しません。
- **会場退出**(`leave`タグ、`state: "leaving"`→`"left"`): 二次会シナリオと同じ、会場そのものから
  離れる動きです。立食パーティーでは、この2つを状態ログ・タグ・Canvas表示のいずれでも混同しない
  ようにしています。

### 輪からの離脱判断について(会話満足度・社交的回遊傾向)

輪からの離脱(責務9)は、`docs/conversation-satisfaction-model.md`に記載のとおり、一定tick(最低滞在tick)
以上の滞在後に、次の2つの独立した要因を合成した確率で発生します(`src/simulation/clusterDepartureDecision.ts`)。

- **満足度低下由来**: 今の会話への満足度(`docs/conversation-satisfaction-model.md`が定義する
  会話満足度モデル)が一定の基準を下回るほど離脱しやすくなります。
- **社交的回遊由来**: 「不満はないが、より多くの人と交流したい」という個体差(`socialCirculationTendency`、
  run中不変の安定的な特性)が高いagentは、満足度が高くても一定の滞在の後は次の輪へ移りやすくなります。

離脱が起きた際は、どちらの要因が支配的だったか(僅差なら「両方」)が構造化ログ(`departureReason`)
から追跡できます。これは実データによる較正を経た心理学的モデルではなく、**性格診断の結果でも
ありません**。誰かが輪から離脱しやすい/しにくいことを、その人の社交性や性格の優劣として解釈しないで
...
#212のmembership区間の時間重複から、2agent間の`ContactIntervalRecord`と無向
...
src/
  ...
  simulation/
    ...
    groupFormation.ts        班人数・教師介入横断のMonte Carlo比較ロジック(`compareGroupFormation`)
    schoolInterventionRuntime.ts 学校向け教師介入の実行契約(hook・context・effect/action/event)
    schoolInterventions/     個別の教師介入(声かけ促進・空き表示・匿名通知・推薦・強制割当・ランダム割当)
    ...
    interventions.ts        二次会・学校の介入シナリオ定義とパラメータ補正・engineロジック
...
教室での班分け・教師介入(本セクション)に関するテストは主に次のファイルにあります。

- `formationPolicy.test.ts` — `FormationPolicy`の共通契約・`afterPartyPolicy`のbyte-identical挙動の検証
- `groupPartition.test.ts` — 定員ルール(固定/可変)からの構造的未割当人数計算のテーブル駆動テスト
- `classroomPair.test.ts` / `classroomPairInvariants.test.ts` — ペア形成の成立・終了・所属整合性の不変条件
- `classroomGroupCapacity.test.ts` — 3人固定・4人固定・3〜4人可変の成立/空き/満員・容量不変条件
- `pairFormation.test.ts` / `groupFormation.test.ts` — 教室シナリオのMonte Carlo集計・介入横断比較
- `schoolInterventionRuntime.test.ts` — hook契約・`InterventionEffect`/`InterventionAction`適用の単体テスト
- `schoolInterventionEngineWiring.test.ts` — `engine.ts`への6 hook結線の検証
- `schoolInterventions.test.ts` — 6種類の教師介入それぞれの発火条件・no-op条件・監査イベントの検証
- `interventions.test.ts` — `resolveAvailableInterventionIds`等、二次会/学校介入カタログの整合性検証
- `nonInterference.test.ts` — no-op介入時に本体PRNG系列・結果が介入なしと一致することの検証
