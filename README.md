...
Phase Aのインスペクター/ログは「1回のシミュレーションを画面で追う」ための機能です。これに加えて、Phase Bでは「1回の実行が最終的にどうなったか」をまとめる終了サマリーと、「同じ条件で何度も試したらどのくらいの確率で起きるか」を見るMonte Carlo集計を追加しました。観察機能は今後もPhase Cとして拡張予定です(詳細は今後のRoadmapで扱います)。

## 観察の流れ

このシミュレーターには、目的が異なる2つの観察手段があります。

- **単発seed(Phase A + 終了サマリー)**: 1つのseedでシミュレーションを動かし、「なぜその動きになったか」を過程ごと読む。インスペクターやログフィルタで途中経過を追い、終了後は終了サマリーで結果を確認する。
- **Monte Carlo集計(Phase B)**: 同じプリセット・パラメータのまま複数のseedで繰り返し実行し、「その条件でどの程度起きやすいか」を確率・平均値として見る。1回ごとの過程は見ないが、たまたまその回だけ起きたのか、その条件で起きやすい傾向なのかを区別できる。

まず単発seedでobserverJoinerの挙動を眺めて仮説を立て、その仮説がその条件でどの程度一般的に起きるのかをMonte Carlo集計で確認する、という順序で使うことを想定しています。
...
src/
  ...
  simulation/
    ...
    monteCarlo.ts           Monte Carlo実行・集計ロジック
  components/
