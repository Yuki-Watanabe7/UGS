...
- **シミュレーション領域**: 円1つ1つが1人のエージェント。色が状態を表します。
  - `gray`: 未定 / `blue`: 二次会に向かう意思が強まりつつある(接近中) / `green`: 参加済み / `red`: 帰宅方向 / `purple`: 核を作っている主導者 / `orange`: observerJoiner型(注目対象)
...
- ログのフィルタリング(特定のエージェントだけ追う、observerJoinerのイベントだけ表示する等)
